...
This will execute and render the notebook `index.ipynb`, then build the publication site. The first execution will take a few minutes since the `_freeze/` cache is not included in the repository; subsequent renders will be much faster as Quarto reuses the cache. To preview the site, use
